...
public class HandlerLoginSucess implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        Study_member studyMember = studyMemberService.doSelectLoginI(authentication.getName());

        // 세션추가
        HttpSession session = request.getSession();
            session.setAttribute("ss_member_id", studyMember.getMemberId());
            session.setAttribute("ss_login_id", studyMember.getLoginId());
            session.setAttribute("ss_name", studyMember.getName());
            session.setAttribute("ss_role", studyMember.getRole());
